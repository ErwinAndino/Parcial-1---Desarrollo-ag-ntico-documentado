...
Repositorio: `https://github.com/ErwinAndino/Parcial-1---Desarrollo-ag-ntico-documentado` (rama `main`; push final al commit `514b0e7`).
...
Declaracion: el repositorio no contiene secretos ni credenciales. `node_modules/` y `dist/` estan excluidos por `.gitignore` y no forman parte del commit de entrega. Toda la evidencia automatizada fue observada (build y tests re-ejecutados); la verificacion interactiva en navegador fue completada por el estudiante el 2026-09-15 según el checklist `docs/prueba-manual-checklist.md`.