...
| Rutas sin recorte de esquina | Local (sin commit) | Test automatizado Node | 1. Ejecutar `route-test.mjs` (2000 rutas aleatorias). | Ningun atajo podado deja el cuerpo del enemigo dentro de una pared; los pasos ortogonales entre celdas se conservan. | Confirmado en la re-ejecucion final: 1999/2000 rutas con camino (1997 en una corrida previa; el conteo varia levemente por aleatoriedad), 0 atajos recortados, 4/4 comprobaciones OK. | Salida del test en Node. |
...
Sistema de esquivada corta con Space: 140 px en 200 ms hacia la ultima direccion de movimiento, enfriamiento de 1 s, invulnerabilidad durante el tramo, recorte contra paredes y bordes del mundo, y HUD con indicador de recarga. El codigo fue validado por `npm run build` (compila, 8 modulos transformados, advertencia de chunk no bloqueante) y por un test automatizado en Node sobre la logica del endpoint (`dash-test.mjs`, modulo puro `src/dash.js`); la verificacion interactiva queda pendiente de prueba manual del estudiante en navegador.

| Criterio | Version validada | Metodo o comando | Pasos | Resultado esperado | Resultado observado | Evidencia |
|---|---|---|---|---|---|---|
| Endpoint en campo abierto | Local (sin commit) | Test automatizado Node | 1. Ejecutar `dash-test.mjs`. | El dash recorre exactamente 140 px en campo abierto (4 direcciones, diagonal, 200 casos aleatorios). | Confirmado: 8 comprobaciones OK (incluye 200 dashes aleatorios a distancia plena dentro del mundo). | Salida del test en Node. |
| Recorte contra paredes | Local (sin commit) | Test automatizado Node | 1. Ejecutar `dash-test.mjs`. | Un muro en el camino detiene el dash en su cara sin penetrarlo; un dash contra muro a contacto no se mueve. | Confirmado: el endpoint queda en la cara del muro (holgura del cuerpo) y el inicio bloqueado queda en el lugar. | Salida del test en Node. |
| Recorte contra bordes del mundo | Local (sin commit) | Test automatizado Node | 1. Ejecutar `dash-test.mjs`. | Ningun endpoint deja el cuerpo fuera del mundo (margen 16 px); 500 dashes aleatorios contra el layout real. | Confirmado: 500 dashes nunca terminan dentro de una pared ni fuera del mundo. | Salida del test en Node. |
| Disparador y cooldown | Local (sin commit) | `npm run dev` + secuencia manual | 1. Pulsar Espacio varias veces seguidas. | Un dash por pulsacion; espera de 1 s antes del siguiente; el HUD muestra `Dash: listo`/`recargando`. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Direccion | Local (sin commit) | `npm run dev` + secuencia manual | 1. Mover con WASD y soltar. 2. Pulsar Espacio sin mover. 3. Pulsar Espacio antes de mover en partida nueva. | Dash hacia la ultima direccion de movimiento; sin input previo, hacia abajo. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Paredes y bordes (manual) | Local (sin commit) | `npm run dev` + secuencia manual | 1. Dash contra varias paredes y bordes del mundo. | El heroe termina en la cara sin atravesar ni trabarse. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Invuln en el tramo | Local (sin commit) | `npm run dev` + secuencia manual | 1. Dash atravesando un enemigo. 2. Esperar que termine el dash en contacto. | Sin perdida de corazones durante el tramo; tras el dash, el daño normal (respetando la invuln de 1,2 s) vuelve a aplicar. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Convivencia con el ataque | Local (sin commit) | `npm run dev` + secuencia manual | 1. Dash durante un barrido. 2. Atacar durante un dash. | Ambos coexisten; el arco sigue al heroe; el ataque conserva su enfriamiento. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Freno en fin de partida | Local (sin commit) | `npm run dev` + secuencia manual | 1. Ganar o perder y pulsar Espacio. | El dash no se dispara; uno en curso queda congelado (R reinicia). | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Regresion Fases 1-4 | Local (sin commit) | `npm run dev` + secuencia manual | 1. Repetir movimiento WASD, ataque, colision, progresion, spawn y rodeo de paredes. | Ningun comportamiento previo se altero con el dash. | Pendiente de prueba manual. | Se completa tras la verificacion del estudiante. |
| Build | Local (sin commit) | `npm run build` | Ejecutar el comando. | Compila sin errores. | Confirmado: compila en ~576 ms, 8 modulos transformados; advertencia de chunk > 500 kB no bloqueante. | Salida completa del comando. |